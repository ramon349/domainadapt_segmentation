...
def get_data_laoders(conf): 
    ...
    if conf['rank']>=0: 
        rank = conf['rank']
        tr_part = partition_dataset(train,num_partitions=dist.get_world_size(),shuffle=False,even_divisible=True,drop_last=False) [rank] # this will create a dataset  for each partion 
        val_part = partition_dataset(val,num_partitions=dist.get_world_size(),shuffle=False,even_divisible=True,drop_last=False)[rank]
    else: 
        tr_part  = train 
        val_part = val   
