...
from monai.transforms import Compose, AsDiscrete, AsDiscreted, Activationsd
...
def infer_loop(model, loader, config, post_transform=None):
    ...
    with torch.no_grad():
        for val_data in tqdm(loader, total=len(loader)):
            val_inputs = val_data[img_k].to(device)
...
def infer_main():
    ...
    with torch.no_grad():
        outs = infer_loop(
            model, test_loader, config=train_conf, post_transform=post_transform
        )
