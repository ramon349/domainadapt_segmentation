...
class DomainPFATrainerRamen(DiceTrainer): 
    ...
    def _build_criterions(self):
        ...
        self.criterions['dice'] = DiceCELoss(to_onehot_y=True)
        self.criterions['proto_loss'] = ProtoLoss(nav_t=0.1,beta=0.001,num_classes=2,device=self.conf['device'][0])
        self.metrics = dict() 
        self.metrics['dice'] = DiceMetric(include_background=False,reduction="mean_batch")
    def train_epoch(self):
        self.model = self.model.train() 
        for i,batch in tqdm(enumerate(self.tr_dl),total=len(self.tr_dl)):
            ...
            t2p_loss,p2t_loss = self.criterions['proto_loss'](self.prototypes,target_f)
...
class DomainPFATrainerRamen(DiceTrainer): 
    ...
    def _load_source_model_weights(self): 
        ...
        self.model.load_state_dict(new_weights)
        self.prototypes = torch.clone(self.model.conv_final[2].conv.weight).view((2,8))
        self.source_prototypes = torch.clone(self.model.conv_final[2].conv.weight).view((2,8))
        for e in self.model.conv_final[2].parameters(): 
            e.requires_grad = False
    def train_epoch(self):
        self.model = self.model.train() 
    ...
    def train_one_step(self,imgs,labels):
        ...
        prob_teacher = F.softmax(pred_teacher,dim=1) #TODO: you  might want to hcange this with softmax 
        pseudo_label_teacher=  torch.argmax(pred_teacher,dim=1)
        #loss calculation 
        drop_percent = self.opt['drop_percent']
        percent_unreliable = (100 - drop_percent) * (1 - self.gb_step / self.total_steps)
