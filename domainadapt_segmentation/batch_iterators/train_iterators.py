...
def eval_loop(model, loader, writer, epoch, device, config):
    ...
    loss_function = DiceLoss(
        include_background=False, reduction="mean", to_onehot_y=True, sigmoid=True
    )
    all_losses = list()
    dice_scores = list()
...
def train_consistency(
    ...
    for batch_n, batch_data in enumerate(train_dl):
        ...
        dice_loss = (
            reduce_tensors(tensor=loss, world_size=dist.get_world_size()).cpu().item()
            if world_size >= 2
            else loss
        )
        consistency_loss = (
            reduce_tensors(tensor=const_loss, world_size=dist.get_world_size())
            .cpu()
            .item()
            if world_size >= 2
            else loss
        ...
        ov_loss = (
            reduce_tensors(tensor=total_loss, world_size=dist.get_world_size())
            .cpu()
            .item()
            if world_size >= 2
            else loss
...
def train_basic_vae(
    ...
    for batch_n, batch_data in enumerate(train_dl):
        ...
        local_rec_loss = (
            reduce_tensors(tensor=recon_loss, world_size=dist.get_world_size())
            .cpu()
            .item()
            if world_size >= 2
            else loss
...
def train_two_branch(
    ...
    for batch_n, batch_data in enumerate(train_dl):
        ...
        embed_domain_loss = (
            criterions["domain"](embed_domain_pred, phase) * has_kidney
        ).mean()
        mask_domain_loss = (
            criterions["domain"](mask_domain_pred, phase) * has_kidney
        ).mean()
        total_loss = (
            seg_loss + 0.0 * embed_domain_loss + 0.0 * mask_domain_loss
        )  # we need the loss to be included in calculation for DDP but grad can be zero
        total_loss.backward()
        optis["task"].step()
        ...
        total_loss = (
            0.0 * seg_loss + domain_loss
        )  # ddp based clipping. if not included you will suffer
        total_loss.backward()
        ...
        embed_conf_loss = torch.mean(
            criterions["conf"](embed_domain_pred, phase) * has_kidney
        )
        mask_conf_loss = torch.mean(
            criterions["conf"](mask_domain_pred, phase) * has_kidney
        )
        seg_loss = criterions["task"](seg_pred, labels)
        ...
        local_domain_embed_loss = reduce_metrics(
            embed_domain_loss, world_size=world_size
        )
        ...
        if writer:
            writer.add_scalar(
                "batch_DICE+CE_loss", local_dice_loss, global_step=global_step_count
            )
            writer.add_scalar(
                "batch_total_loss", local_loss, global_step=global_step_count
            )
            writer.add_scalar(
                "embed_domain_loss",
                local_domain_embed_loss,
                global_step=global_step_count,
            )
            writer.add_scalar(
                "mask_domain_loss",
                local_domain_mask_loss,
                global_step=global_step_count,
            )
            writer.add_scalar(
                "embed_confusion_loss",
                local_conf_embed_loss,
                global_step=global_step_count,
            )
            writer.add_scalar(
                "mask_confusion_loss",
                local_conf_mask_loss,
                global_step=global_step_count,
            )
        global_step_count += 1
    epoch_loss /= step
...
def reduce_metrics(tens, world_size):
    out = (
        reduce_tensors(tensor=tens, world_size=world_size) if world_size >= 2 else tens
    )
    return out
