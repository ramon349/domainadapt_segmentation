...
@ModelRegister.register("UnetDsBN2D")
class Unet2D(nn.Module):
    def __init__(self,spatial_dims=2,in_channels=1,out_channels=2,): 
        assert spatial_dims ==2
        super(Unet2D, self).__init__()
        c=in_channels
        num_classes=out_channels
        n= 16  #number of channel scaling 
        norm ='bn' 
        num_domains = 4 
        momentum=0.1
    ...
    def forward(self, x, weights=None, domain_label=None):
        ...
        return predictions
    def predictor_wrapper(self,data,domain_id=None): 
        # this is a wrapper for running inference on the sliding window inference 
        data = data.squeeze(-1)  #we have to squeeze cause sliding window will return (512,512,1)

        domain_label = domain_id * torch.ones(data.shape[0], dtype=torch.long, device=data.device)  # need to identify l domain as gpu tensor
        
        output = self.forward(data, domain_label=domain_label).unsqueeze(-1) #run inference 
