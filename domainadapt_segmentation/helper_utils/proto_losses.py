...
class ProtoLoss(nn.Module):
    ...
    def __init__(self, nav_t: float, beta: float, num_classes: int, device: torch.device, s_par: Optional[float] = 0.5, reduction: Optional[str] = 'mean'):
        ...
        self.s_par = s_par
        self.beta = beta
        self.prop = (torch.ones((num_classes,1))*(1/num_classes))#change all props to modify the other element
    ...
    def get_pos_logits(self, sim_mat, prop):
        ...
        log_prior = torch.log(prop.to(device) + self.eps)
    ...
    def forward(self, mu_s: torch.Tensor, f_t: torch.Tensor) -> torch.Tensor:
        ...
        old_logits = self.get_pos_logits(sim_mat.detach(), self.prop.to(device))
